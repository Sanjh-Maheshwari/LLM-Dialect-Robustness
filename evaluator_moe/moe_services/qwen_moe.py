...
class QwenMOEClassifier:
    ...
    def __init__(self, model_id="Qwen/Qwen2.5-7B-Instruct", lora_path="./output/mixlora/qwen"):
        ...
        try:
            ...
            logger.error(f"Error loading Qwen MOE model: {e}")
            self.model = None
    ...
    def predict(self, instruction: str, context: str, dialect: str, task: str, domain: str, max_new_tokens: int = 50) -> str:
        ...
        lora_weights = os.path.join(self.lora_path, f"mixlora_qwen_{task.lower()}_{domain.lower()}_0")
    ...
    def predict_fewshot(self, instruction: str, context: str, few_shot_examples: list,
        ...
        try:
            ...
            logger.error(f"Error in Qwen MOE few-shot prediction for dialect {dialect}: {e}")
            return '0'
