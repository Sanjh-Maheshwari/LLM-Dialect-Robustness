"""
Few-shot evaluation script for MixLoRA (MOE) models
Supports 5 models: Phi-3, Mistral Small, Qwen, LLaMA, Gemma 2

Each model uses MixLoRA adapters trained on dialect-specific data.
...
Usage:
    python evaluate_moe_fewshot.py --model mistral
    python evaluate_moe_fewshot.py --model gemma
    python evaluate_moe_fewshot.py --model qwen
    python evaluate_moe_fewshot.py --model phi
    python evaluate_moe_fewshot.py --model llama
"""
...
from evaluator_moe_fewshot.moe_services.mistral_moe import MistralMOEClassifier
from evaluator_moe_fewshot.moe_services.gemma_moe import GemmaMOEClassifier
from evaluator_moe_fewshot.moe_services.qwen_moe import QwenMOEClassifier
from evaluator_moe_fewshot.moe_services.phi_moe import PhiMOEClassifier
from evaluator_moe_fewshot.moe_services.llama_moe import LlamaMOEClassifier
...
def sample_fewshot_examples(dialect_df, current_idx, num_shots=NUM_SHOTS):
    ...
    Uses deterministic seeding for reproducibility across models
...
def evaluate_dialect_fewshot(model, variety, task, domain, json_path):
    """Evaluate MOE model on a specific dialect using few-shot prompting"""
